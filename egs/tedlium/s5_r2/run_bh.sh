...
if [ $stage -le 2 ]; then
  ...
  local_bh/prepare_dict.sh --train_set train --lexicon $lexicon --g2p-model $g2p_model
fi
...
if [ $stage -le 4 ]; then
  ...
  lm_dir=data/local/local_lm/data/arpa
  mkdir -p $lm_dir
  # use train corpus to generate language model
  cp data/train/oov/text_clean_lm $lm_dir

  # 3-gram
  # ngram-count -order 3 -text $lm_dir/text_clean_lm -kndiscount -interpolate -lm $lm_dir/3gram_small.arpa.gz
  # 4-gram
  ngram-count -order 4 -text $lm_dir/text_clean_lm -kndiscount -interpolate -lm $lm_dir/4gram_small.arpa.gz
...
if [ $stage -le 6 ]; then
  ...
  for set in test train; do
    ...
    steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" \
      ...
      --mfcc-config conf/mfcc.conf $dir
...
if [ $stage -le 15 ]; then
  ...
  steps/train_sat.sh --cmd "$train_cmd" \
    15000 300000 data/train data/lang exp/tri2_ali exp/tri3
...
if [ $stage -le 19 ]; then
  py_w2v="/home/bhuang/anaconda3/envs/asr/bin/python"
  model=data/models/wav2vec2-FR-7K-large.pt
  # model=data/models/xlsr_53_56k.pt  
  # for dset in dev test train_cleaned_sp; do
  for dset in test train_cleaned_sp; do
    # utils/copy_data_dir.sh data/$dset data/${dset}_${data_affix}

    # utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    # ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
    # $train_cmd --gpu 1 --num-threads 12 --time 24:00:00 data/${dset}_${data_affix}/log/extract_wav2vec2.$dset.log \
    # $py_w2v local/extract_wav2vec.py $model data/${dset} data/${dset}_${data_affix} || exit 1

    # data_affix=w2v_layer24
    # utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    # ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
    # data_affix=w2v_layer19
    # utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    # ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
    # data_affix=w2v_layer22
    # utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    # ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
    # data_affix=w2v_features
    # utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    # ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
    data_affix=w2v_layer11
    utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
    data_affix=w2v_layer2
    utils/copy_data_dir.sh data/$dset /projects/bhuang/data/${dset}_${data_affix}
    ln -s /projects/bhuang/data/${dset}_${data_affix} data/${dset}_${data_affix}
...
if [ $stage -le 20 ]; then
  ...
  local/chain/run_tdnn_w2v.sh --data-affix ${data_affix} || exit 1
fi
