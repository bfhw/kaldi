...
# tdnn_affix=1g_${data_affix}  #affix for TDNN directory, e.g. "a" or "b", in case we change the configuration.
...
utils/data/combine_short_segments.sh \
     data/${train_set}_sp_${data_affix} $min_seg_len data/${train_set}_sp_${data_affix}_comb || exit 1
...
train_data_dir=data/${train_set}_sp_${data_affix}_comb
...
if [ $stage -le 18 ]; then
  ...
  # bh: --trainer.frames-per-iter 500000
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --egs.opts "--frames-overlap-per-eg 0 --constrained false --max-shuffle-jobs-run 10" \
  ...
  for dset in test; do
    (
      steps/nnet3/decode.sh --num-threads 4 --nj $decode_nj --cmd "$decode_cmd" \
         ...
         $dir/graph data/${dset}_${data_affix} $dir/decode_${dset} || exit 1;
