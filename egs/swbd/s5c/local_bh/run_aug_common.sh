...
# b/ gen low-resolution MFCC for ali
if [ $stage -le 2 ]; then
...
# b/ duplicate alignments for augmented data
...
# b/ gen hires mfcc of training sets for ivector and following training
if [ $stage -le 4 ]; then
  mfccdir=mfcc_hires
  # if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $mfccdir/storage ]; then
  #   date=$(date +'%m_%d_%H_%M')
  ...
  for dataset in ${train_set}_aug; do
    ...
    # b/ volume perturbation on all uut between scale_low=0.125 and scale_high=2
...
# b/ gen hires mfcc of test sets for ivector and following training
if [ $stage -le 5 ]; then
  mfccdir=mfcc_hires
...
if [ "$use_ivectors" == "true" ]; then
  if [ $stage -le 6 ]; then
    ...
    # b/ get subset of augmented data to train i-vector
    ...
    # b/ remove sp in ivector training data if sp has been done in the previous steps, sp shouldn't add new speakers
    sed -i '/sp0.9\|sp1.1/d' data/${ivector_trainset}_aug_hires/utt2spk
    utils/fix_data_dir.sh data/${ivector_trainset}_aug_hires

    # b/ downsample to required size
  ...
  if [ $stage -le 7 ]; then
    ...
    echo "$0: Creating alignments of aug data by copying alignments of clean data"
    # b/ copy ali for ivector training set
    steps/copy_ali_dir.sh --nj $nj --cmd "$train_cmd" \
    ...
    # b/ use hires mfcc for train_lda_mllt, train_diag_ubm and train_ivector_extractor
