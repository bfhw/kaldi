...
# based on local/nnet3/run_ivector_common.sh
...
if $speed_perturb; then
  ...
  if [ $stage -le 2 ] && $generate_alignments; then
    ...
    steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
      ...
      data/${train_set}_sp data/lang exp/${ali_model} exp/${ali_model}_ali_nodup_sp
  fi
fi
...
if [ $stage -le 3 ]; then
  ...
  for dataset in $train_set $ivector_trainset; do
...
if [ $stage -le 5 ]; then
  ...
  # * don't do sp on ivector training set
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    --splice-opts "--left-context=3 --right-context=3" \
    5500 90000 data/${ivector_trainset}_hires \
    data/lang exp/${ivector_trainset_ali} exp/nnet3/tri3b
fi
...
if [ $stage -le 7 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj $nj \
    data/${ivector_trainset}_hires exp/nnet3/diag_ubm exp/nnet3/extractor || exit 1;
fi
