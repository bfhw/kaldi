...
if [ $stage -le 0 ]; then
  ...
  if [ ! -f data/$clean_set/feats.scp ]; then
    steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj \
      data/$clean_set exp/make_mfcc/$clean_set $mfccdir || exit 1
    ...
    # attention here
    echo "MFCC features of $clean_set already exist"
  fi
fi
...
if $multi_condition; then
  ...
  if $speed_perturb; then
    ...
    if [ $stage -le 3 ]; then
      ...
      steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj \
        data/${clean_set}_sp exp/make_mfcc/${clean_set}_sp $mfccdir
    ...
    if [ $stage -le 4 ]; then
      ...
      #   data/${clean_set}_hires exp/nnet3/extractor exp/nnet3/ivectors_pre_${clean_set} || exit 1;
      ...
      #   --online-ivector-dir exp/nnet3/ivectors_pre_${clean_set} \
      #   data/${clean_set}_hires data/lang exp/chain/tdnn7r_sp_aug exp/${clean_lat}_sp || exit 1
    fi
  ...
  if [ $stage -le 5 ]; then
    ...
    # duplicate ali into ${clean_ali}_aug
    # train ivector on mutli conditions but not speed
  ...
  if [ $stage -le 8 ]; then
    ...
    # build tree use ali, training use lats
...
if [ $stage -le 11 ]; then
  ...
  # ? alignment-subsampling-factor no need for gmm ali but for tdnn
